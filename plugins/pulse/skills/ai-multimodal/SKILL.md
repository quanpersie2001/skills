...
- Works without external helper modules; centralized resolver support is optional
...
- **Bundled CLI**
  ...
  - `media_optimizer.py`
  - `check_setup.py`
- **Reference-only SDK features**
  - TTS and native audio generation
  - image editing and multi-turn refinement
  - search-grounded image generation
  - YouTube analysis, video clipping, custom FPS, multi-video comparison
  - structured JSON schema output
  - live music generation with Lyria

Do not assume every capability documented in `references/` is already exposed by the CLI.
