...
try:
    ...
    class KeyRotator:
        """Minimal self-contained API key rotator."""

        def __init__(self, keys, cooldown_seconds: int = 60, verbose: bool = False):
            self.keys = keys
        ...
        def mark_rate_limited(self, error_text: str = "") -> bool:
            if not self.current_key:
                return False
            self.cooldowns[self.current_key] = time.time() + self.cooldown_seconds
            if self.verbose:
                print(
                    f"⚠ Key cooled down for {self.cooldown_seconds}s: {self.current_key[:8]}...",
                    file=sys.stderr
                )
            return True
    ...
    def find_all_api_keys() -> List[str]:
        keys = []
        primary = find_api_key()
        if primary:
            keys.append(primary)
        ...
        for env_name in sorted(os.environ):
            if not env_name.startswith('GEMINI_API_KEY_'):
                continue
            suffix = env_name.replace('GEMINI_API_KEY_', '', 1)
...
def load_local_env_files() -> None:
    """Load local .env files from repo-like parents when dotenv is available."""
    if 'load_dotenv' not in globals() or load_dotenv is None:
        return
    ...
    candidates = []
    for path in [
        skill_dir.parent.parent,  # repo root or broader shared config
        skill_dir.parent,         # shared standalone skills config
        skill_dir,                # skill-specific config
...
def infer_task_from_file(file_path: str) -> str:
    ...
    document_extensions = {'.pdf', '.txt', '.html', '.htm', '.md', '.csv', '.doc', '.docx', '.pptx', '.xlsx'}
...
def detect_stdin_input() -> Optional[Dict[str, Any]]:
    """Read supported binary payloads from stdin when piped."""
    ...
    if not mime_type:
        raise ValueError(
            "Unsupported stdin input. Supported stdin formats: PNG, JPG, PDF, WAV, MP3, WEBP."
        )
...
def infer_task_from_mime_type(mime_type: str) -> str:
...
def materialize_stdin_to_tempfile(stdin_input: Dict[str, Any]) -> str:
    temp_file = tempfile.NamedTemporaryFile(
        prefix='ai_multimodal_stdin_',
        suffix=stdin_input['suffix'],
        delete=False
    )
    temp_file.write(stdin_input['data'])
    temp_file.flush()
    temp_file.close()
...
Examples:
  ...
  %(prog)s --task generate --prompt "Product photo of coffee mug" \\
    ...
    parser.add_argument('--reference-images', nargs='+',
                       help='Reference images for video generation (current implementation uses first frame, optional last frame)')
    ...
    stdin_input = detect_stdin_input()

    # Auto-detect task from file type or stdin if not specified
    if not args.task:
        if args.files and len(args.files) > 0:
            ...
            args.task = infer_task_from_mime_type(stdin_input['mime_type'])
            if args.verbose:
                print(f"Auto-detected task: {args.task} (from stdin MIME type)")
        else:
    ...
    # Validate arguments
    if args.task not in ['generate', 'generate-video'] and not args.files and not stdin_input:
    ...
    if stdin_input:
        temp_path = materialize_stdin_to_tempfile(stdin_input)
        cleanup_paths.append(temp_path)
        files.append(temp_path)
        if args.verbose:
            print(f"Using stdin payload via temp file: {temp_path}")
