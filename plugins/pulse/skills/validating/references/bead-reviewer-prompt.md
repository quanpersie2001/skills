...
This reviewer can also be used as a lightweight pre-close sanity-check during validating. In that mode, focus on evidence, testing mode, and standalone clarity rather than broader redesign.
...
Assume the canonical Pulse bead schema. If a bead is missing core structured fields such as `files`, `dependencies`, `verify`, `verification_evidence`, or `testing_mode`, treat that as a bead defect, not as something to recover from by guessing.
...
- `verification_evidence` points only to vague prose instead of a real artifact path or explicit record
...
- `verification_evidence: history/auth-refresh/verification/br-012.md` with captured output or a recorded result

### CRITICAL Pattern 6: Missing Canonical Schema Fields

The bead is missing structured fields that downstream skills depend on.

**Fail examples:**
- No `files:` field
- No `dependencies:` field
- No `decision_refs:` field when the bead clearly implements locked decisions
- No `learning_refs:` field even though the bead body references prior learnings
- No `verification_evidence:` field
- No `testing_mode:` field

### CRITICAL Pattern 7: TDD Contract Missing

The bead says `testing_mode: tdd-required` but does not provide an explicit red-green path.

**Fail examples:**
- `testing_mode: tdd-required` with only a generic `verify` step
- `testing_mode: tdd-required` with no `tdd_steps`
- `tdd_steps` present but red and green commands are indistinguishable

**Pass example:**
- `testing_mode: tdd-required` plus `tdd_steps.red` that fails before implementation and `tdd_steps.green` that passes after minimal code

**Schema pass example:**
- The bead includes explicit `dependencies`, `files`, `verify`, `verification_evidence`, `testing_mode`, `decision_refs`, and `learning_refs` fields, even when some are empty lists.
