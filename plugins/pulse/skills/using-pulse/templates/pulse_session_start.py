...
def find_repo_root(start: str) -> Path:
    ...
    for candidate in [path, *path.parents]:
        if (candidate / ".pulse" / "onboarding.json").exists():
            return candidate
...
def main() -> int:
    ...
    if onboarding_path.exists():
        notes.append(
            "Pulse onboarding is installed for this repo. Read AGENTS.md before substantive work."
        )
    else:
        notes.append(
            "Pulse onboarding is missing in this repo. Load pulse:using-pulse before continuing."
        )
