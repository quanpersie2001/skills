...
USING_PULSE_DIR = SCRIPT_PATH.parent.parent
PLUGIN_ROOT = USING_PULSE_DIR.parent.parent
PLUGIN_MANIFEST_PATH = PLUGIN_ROOT / ".codex-plugin" / "plugin.json"
AGENTS_TEMPLATE_PATH = PLUGIN_ROOT / "AGENTS.template.md"
HOOK_TEMPLATES_DIR = USING_PULSE_DIR / "templates"
ONBOARDING_SCHEMA_VERSION = "1.0"
COMPACT_PROMPT_MARKER_START = "# PULSE: compact_prompt start"
COMPACT_PROMPT_MARKER_END = "# PULSE: compact_prompt end"
...
def managed_agents_present(text: str) -> bool:
    return "<!-- PULSE:START -->" in text and "<!-- PULSE:END -->" in text
        ...
        "SessionStart": [
            {
                ...
                "hooks": [
                    {
                        "type": "command",
                        "command": MANAGED_HOOK_COMMAND_PREFIX + 'pulse_session_start.py"',
        ...
        "PreToolUse": [
            {
                ...
                "hooks": [
                    {
                        "type": "command",
                        "command": MANAGED_HOOK_COMMAND_PREFIX + 'pulse_pre_tool_use.py"',
        ...
        "Stop": [
            {
                "hooks": [
                    {
                        "type": "command",
                        "command": MANAGED_HOOK_COMMAND_PREFIX + 'pulse_stop.py"',
...
def is_pulse_hook_entry(entry: Dict) -> bool:
    for hook in entry.get("hooks", []):
        command = hook.get("command", "")
        status = hook.get("statusMessage", "")
        if "/.codex/hooks/pulse_" in command or status.startswith("Pulse:"):
            return True
    return False
...
def merge_hooks_json(path: Path) -> Tuple[str, List[str]]:
    ...
    for event_name, entries in managed.items():
        ...
        filtered = [entry for entry in current_entries if not is_pulse_hook_entry(entry)]
...
def write_hook_scripts(repo_root: Path) -> List[str]:
    ...
    written = []
    for name in ("pulse_session_start.py", "pulse_pre_tool_use.py", "pulse_stop.py"):
...
def check_repo(repo_root: Path) -> Dict:
    ...
    onboarding_path = repo_root / ".pulse" / "onboarding.json"
    ...
    for entries in hooks_data.get("hooks", {}).values():
        if any(is_pulse_hook_entry(entry) for entry in entries):
    ...
    if not agents_exists:
        ...
        actions.append("append_pulse_managed_block_to_AGENTS.md")
    ...
    if not config_path.exists():
        ...
        if compact_prompt_conflict:
            ...
            actions.append("install_pulse_compact_prompt")

    if not hooks_path.exists():
        actions.append("create_.codex/hooks.json")
    if not has_managed_hooks:
        actions.append("install_pulse_hook_entries")

    if onboarding.get("plugin_version") != plugin_version:
        actions.append("write_.pulse/onboarding.json")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Check or apply Pulse repo onboarding.")
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Replace an existing non-Pulse compact_prompt.",
    )
