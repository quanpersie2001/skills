...
import onboard_pulse


class OnboardPulseTests(unittest.TestCase):
    def test_apply_creates_full_repo_onboarding(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            result = onboard_pulse.apply_repo(root, allow_compact_prompt_replace=False)

            self.assertEqual(result["result"]["status"], "complete")
            self.assertEqual(result["status"], "up_to_date")
            self.assertTrue((root / "AGENTS.md").exists())
            self.assertIn("Pulse Workflow", (root / "AGENTS.md").read_text(encoding="utf-8"))
            self.assertTrue((root / ".codex" / "config.toml").exists())
            self.assertTrue((root / ".codex" / "hooks.json").exists())
            self.assertTrue((root / ".pulse" / "onboarding.json").exists())
    ...
    def test_apply_appends_managed_block_to_existing_agents(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            onboard_pulse.apply_repo(root, allow_compact_prompt_replace=False)
            agents_text = (root / "AGENTS.md").read_text(encoding="utf-8")

            self.assertIn("# Existing instructions", agents_text)
            self.assertIn("<!-- PULSE:START -->", agents_text)
            self.assertEqual(agents_text.count("<!-- PULSE:START -->"), 1)
