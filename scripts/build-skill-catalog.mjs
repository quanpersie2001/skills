...
const outputJsonPath = path.join(repoRoot, "skill-catalog.json");
...
function printHelp() {
  console.log(`Build Pulse skill catalog artifact

Usage:
  node scripts/build-skill-catalog.mjs [--check]

Options:
  --check   Validate generated file is current and exit non-zero on drift
  --help    Show this help
`);
}
...
function buildJson(skills) {
  return {
    schemaVersion: 1,
    sourceRoot: "skills",
    generatedFiles: ["skill-catalog.json"],
...
function main() {
  ...
  if (args.check) {
    ...
    if (!jsonCheck.ok) {
      console.error(jsonCheck.reason);
      process.exit(1);
    }
  ...
  console.log("wrote skill-catalog.json");
}
