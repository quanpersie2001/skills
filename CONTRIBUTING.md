...
- The Codex package manifest lives at [`.codex-plugin/plugin.json`](.codex-plugin/plugin.json). It declares the plugin name, version, interface metadata, and packaged runtime contract.
...
4. Codex discovers skills automatically from the `./skills/` directory declared in [`.codex-plugin/plugin.json`](.codex-plugin/plugin.json), so no per-skill manifest change is needed.
